pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        git branch: 'main', url: 'https://github.com/Madhu-123-bot/mondayproj-1.git'
      }
    }
    ...
    stage('Build Docker Image') {
      steps {
        script {
          dockerImage = docker.build("containerguru1/mondayproj-1")
        }
      }
    }
    ...
    stage('Deploy to AWS EC2') {
      steps {
        script {
          ...
          sh 'chmod 600 /var/lib/jenkins/workspace/mondayproj-1/ssh8633610058472328627.key'
          ...
          withEnv(["PATH+ANSIBLE=/usr/local/bin:/usr/bin"]) {
            ...
            sh '''
              ansible-playbook playbook.yml -i hosts.txt --private-key /var/lib/jenkins/workspace/mondayproj-1/ssh8633610058472328627.key -u ubuntu
            '''
          }
        }
      }
